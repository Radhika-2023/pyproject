...
def create_location(request: Locations.CreateLocation, db: Session = Depends(get_db)):
    new_location = models.Locations(**request.model_dump())
    db.add(new_location)
    db.commit()
...
def update_location(update_location_req: Locations.LocationsBase, request_id: int, db: Session = Depends(get_db)):
    update_location_req = db.query(models.Locations).filter(models.Locations.id == request_id)

    if update_location_req.first() is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"The id:{request_id} does not exist")
    update_location_req.update(update_location_req.dict(), synchronize_session=False)
    db.commit()

    return update_location_req.first()
